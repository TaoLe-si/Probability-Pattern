...
<ItemLink id="probabilitypattern:probability_pattern_terminal" /> 是用于编码概率样板的线缆终端。在**合成 / 切石 / 锻造**模式下与原版 <ItemLink id="ae2:pattern_encoding_terminal" /> 完全一致；在**处理**模式下，它编码的是**概率样板**，而非普通处理样板。
