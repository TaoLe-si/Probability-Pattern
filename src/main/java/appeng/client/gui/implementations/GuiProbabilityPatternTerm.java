...
public class GuiProbabilityPatternTerm extends GuiMEMonitorable
{
	...
	public void initGui()
	{
		...
		this.probabilityField = new GuiTextField( this.fontRendererObj, this.guiLeft + 8, this.guiTop + this.ySize - 154, 56, 14 );
	...
	protected void actionPerformed( final GuiButton btn )
	{
		...
		else if( btn == this.alphaButton )
		{
			...
			ProbabilityPatternNetwork.CHANNEL.sendToServer( new ProbabilityPatternPacket( Action.SET_ALPHA95, newAlpha95 ? 1 : 0 ) );
		}
	}
	...
	protected void keyTyped( final char character, final int key )
	{
		if( this.probabilityField.textboxKeyTyped( character, key ) )
		{
			...
			if( parsed != null && Math.abs( parsed - this.lastSentProbability ) > 1.0e-6 )
			{
				...
				ProbabilityPatternNetwork.CHANNEL.sendToServer( new ProbabilityPatternPacket( Action.SET_PROBABILITY, parsed ) );
			}
			return;
		}
	...
	public void drawFG( final int offsetX, final int offsetY, final int mouseX, final int mouseY )
	{
		...
		final String title = StatCollector.translateToLocal( "container.probabilitypattern.probability_pattern_terminal" );
		...
		this.alphaButton.displayString = StatCollector.translateToLocal( this.container.alpha95Flag != 0 ? "gui.probabilitypattern.alpha95" : "gui.probabilitypattern.alpha99" );
	}
