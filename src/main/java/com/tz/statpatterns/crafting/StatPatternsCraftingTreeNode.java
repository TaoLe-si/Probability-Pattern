 ...
 */
public class StatPatternsCraftingTreeNode {

    @Nullable
    final IPatternDetails.IInput parentInput;
    private final StatPatternsCraftingCalculation job;
    private final StatPatternsCraftingTreeProcess parent;
    private final Level level;
    private final AEKey what;
    private final long amount;
    private ArrayList<StatPatternsCraftingTreeProcess> nodes = null;
    private final boolean canEmit;
    private long probabilityTotalRequested;

    public StatPatternsCraftingTreeNode(ICraftingService cc, StatPatternsCraftingCalculation job, AEKey what, long amount,
            StatPatternsCraftingTreeProcess par, int slot) {
        this.parent = par;
            ...
            if (gridNode != null) {
                ...
                for (var details : wrapPatternsForNode(craftingService, this.what)) {
                    if (this.parent == null || this.parent.notRecursive(details)) {
                        this.nodes.add(new StatPatternsCraftingTreeProcess(craftingService, job, details, this));
                    }
                }
            }
        }
    ...
    void request(CraftingSimulationState inv, long requestedAmount,
        ...
        if (this.nodes.size() == 1) {
            final StatPatternsCraftingTreeProcess pro = this.nodes.get(0);
            ...
            for (StatPatternsCraftingTreeProcess pro : this.nodes) {
                try {
                    while (pro.possible && totalRequestedItems > 0) {
    ...
    long getNodeCount() {
        long tot = 1;
        if (this.nodes != null) {
            for (StatPatternsCraftingTreeProcess pro : this.nodes) { tot += pro.getNodeCount(); }
        }
        return tot;
    }
