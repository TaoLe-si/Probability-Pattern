...
public final class EncodedStatisticalPattern
{
	...
	public EncodedStatisticalPattern( final List<ItemStack> inputsPerAttempt, final ItemStack output, final double successProbability, final double alpha, final int smallSampleLimit, final boolean isAlpha95 )
	{
	...
	public static EncodedStatisticalPattern decode( final NBTTagCompound tag )
	{
		...
		return new EncodedStatisticalPattern( inputs, output, tag.getDouble( TAG_SUCCESS_PROBABILITY ), tag.getDouble( TAG_ALPHA ), tag.getInteger( TAG_SMALL_SAMPLE_LIMIT ), tag.getBoolean( TAG_ALPHA95 ) );
	}
}
