...
public class ContainerProbabilityPatternTerm extends ContainerMEMonitorable implements IOptionalSlotHost
{
	...
	public ContainerProbabilityPatternTerm( final InventoryPlayer ip, final ITerminalHost monitorable )
	{
		...
		{
			throw new IllegalArgumentException( "Probability Pattern Terminal container requires a ProbabilityPatternTerminalPart host." );
		}
		...
		for( int y = 0; y < 3; y++ )
		{
			for( int x = 0; x < 3; x++ )
			{
				this.craftingSlots[x + y * 3] = new SlotFakeCraftingMatrix( craftingInv, x + y * 3, 18 + x * 18, -76 + y * 18 );
		...
		this.patternSlotIN = new SlotRestrictedInput( SlotRestrictedInput.PlacableItemType.PATTERN, patternInv, 0, 147, -72 - 9, this.getInventoryPlayer() );
		this.patternSlotOUT = new SlotRestrictedInput( SlotRestrictedInput.PlacableItemType.ENCODED_PATTERN, patternInv, 1, 147, -72 + 34, this.getInventoryPlayer() );
	...
	public void encode()
	{
		...
		final ItemStack encoded = StatisticalPatternDetails.encode( inputs, output, probability, alpha, this.patternTerminal.isAlpha95() );
