 ...
 */
package com.zincglux.statpatterns.part;
...
/**
 * Cable-attached ME Probability Pattern Encoding Terminal (Spec v2.0 3.3.1).
 * <p>
 * Extends AE2's {@link PartPatternTerminal} (wired pattern terminal part) and adds the
 * probability parameters — single-attempt success probability {@code p} (default 0.8)
 * and the {@code alpha95} confidence flag (default true = 95%). They are persisted in
 * the part NBT and baked into every encoded probability pattern.
 */
public class ProbabilityPatternTerminalPart extends PartPatternTerminal {
    ...
    /** Encoding-grid slot index the amount dialog is currently editing (set by the OPEN packet). */
    private int pendingValueIndex = 0;
    /** Item being adjusted by the amount dialog (cached by the OPEN packet, not persisted). */
    private ItemStack pendingValueStack;
    ...
    public int getPendingValueIndex() {
        return this.pendingValueIndex;
    }

    public void setPendingValueIndex(final int valueIndex) {
        this.pendingValueIndex = valueIndex;
    }

    public ItemStack getPendingValueStack() {
        return this.pendingValueStack;
    }

    public void setPendingValueStack(final ItemStack stack) {
        this.pendingValueStack = stack;
    }
    ...
    public boolean onPartActivate(final EntityPlayer player, final Vec3 pos) {
        ...
        // Encode the part side in the gui id so the gui handler can resolve the exact
        // part on the cable bus.
